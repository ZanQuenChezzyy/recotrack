...
class Chezzy extends Command
{
    ...
    public function handle()
    {
        ...
        $tasks = [
            ...
            'Menginstall NPM dependencies' => fn() => $this->runProcess('npm install'),
            'Melakukan NPM build' => fn() => $this->runProcess('npm run build'),
        ];

        $messages = [
            'Menyalin .env & generate key' => ['<fg=cyan>Berhasil Menyalin .env & generate key.</>', '<fg=yellow>.env & Key sudah ada, tidak perlu diubah.</>'],
            'Menginstall NPM dependencies' => ['<fg=cyan>Berhasil Menginstall NPM dependencies.</>', '<fg=yellow>Dependencies npm sudah ada, tidak perlu diulang.</>'],
            'Melakukan NPM build' => ['<fg=cyan>Berhasil build.</>', '<fg=yellow>Build telah dilakukan, tidak perlu diulang.</>'],
        ];
